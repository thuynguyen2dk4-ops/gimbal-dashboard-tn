...
import { Bluetooth, X, Zap, ZapOff, Grid, SwitchCamera, ScanFace, Image as ImageIcon, Download } from 'lucide-react';
...
export const CameraView = ({ 
  ...
  const [isModelLoading, setIsModelLoading] = useState(false); // <-- Dòng này vừa được thêm lại
        ...
        request.onsuccess = () => {
          ...
          request.result.forEach(item => {
            if (now - item.id > oneWeek) {
              ...
              // CÁCH SỬA LỖI: Tạo URL 1 lần duy nhất lúc load data
              validItems.push({
                ...item,
  ...
  const saveToDB = async (blob, type) => {
    ...
    // CÁCH SỬA LỖI: Tạo URL ngay khi lưu file
    const itemWithUrl = {
      ...newItem,
      url: URL.createObjectURL(blob)
    };
    
    setGalleryItems(prev => [itemWithUrl, ...prev]);
    setLastMedia(itemWithUrl.url);
  };
  ...
  const toggleAiTracking = async () => {
    ...
    setIsModelLoading(true); // Bắt đầu xoay icon loading
    try {
    ...
    setIsModelLoading(false); // Dừng xoay icon loading
  };
